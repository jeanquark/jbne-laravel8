<template>
    <div>
        ...
        data: {{ data }}<br /><br />
        <!-- data3: {{ data3 }}<br /><br /> -->
        <!-- <div v-for="(item, index) in data3" :key="index">{{ item }}<br /></div> -->
        <!-- <br /><br /> -->
        ...
        >
            ...
            <template v-slot:label="{ item }">
                <span v-if="item.file" class="file" @click="download(item)">
                    {{ item.name }} <!--<v-btn small @click="download(item)">download</v-btn>-->
                </span>
                <span v-else>
                    {{ item.name }}
                </span>
        ...
        for (let i = 0; i < data.length; i++) {
            ...
            for (let j = 0; j < separator.length; j++) {
                ...
                let parentId = overrideParentId
                    ? newParentId
                    : parseInt(`${j < 1 ? 0 : j}${j < 1 ? 0 : i + 1}`);
                let double = this.entries3.find(
                    a => a.name === separator[j] && a.parentId == parentId
                );
                if (double) {
